        ...
        for (int cursor_col=0; cursor_col < occupancy_grid.cols(); cursor_col++) // scan each column
        {
            ...
            if ((occupancy_grid(cursor_row, cursor_col)==100)&&((occupancy_grid(cursor_row-1, cursor_col)+occupancy_grid(cursor_row+1, cursor_col)+occupancy_grid(cursor_row, cursor_col-1)+occupancy_grid(cursor_row, cursor_col+1))<250)&&(occupancy_grid(cursor_row, cursor_col+1)==100)) // if the cell is occupied -> it may be new obstacle // TODO: Add condition if at the edge of occupancy grid
            {
                ...
                if (flag_not_in_border) // the detected cell is not part of a border, it means we have hit a new obstacle
                {
                    ...
                    //std::cout << cell_robot.transpose() << std::endl;
                    ...
                    if ((x_min<cell_robot(0,0))&&(x_max>cell_robot(0,0))&&(y_min<cell_robot(1,0))&&(y_max>cell_robot(1,0)))
                    {
                        ...
                        for (int i_obs=0; i_obs<obstacle.rows(); i_obs++)
                        {
                            ...
                            if ((obstacle(i_obs,1)==cell_robot(1,0))&&(obstacle(i_obs,0)<x_index_max)) // same row as the robot
                            {
                        ...
                        if (x_index_max == (x_max+1)) {throw std::logic_error("Obstacle has no occupied cell on the row of the robot");}
                        ...
                        /*std::cout << "Entering display border" << std::endl;
                        display_border( obstacle, border_obstacle);*/
                        ...
                        //std::cout << "Occupancy grid after detecting the enclosing obstacle" << std::endl;
                        //std::cout << occupancy_grid.block(cell_robot(0,0)-20, cell_robot(1,0)-20, 41, 41) << std::endl;
...
Eigen::MatrixXf weights_special(State const& state_robot, Eigen::MatrixXf const& mat_gamma, int const& method, float const& limit_distance, bool const& do_not_normalize)
...
float get_max_gamma_distance(Eigen::Matrix<float, 1, 2> const& proj_robot, Eigen::Matrix<float,1,2> const& normal, Border const& border) // maximum gamma distance that can be reached following the normal
{
    ...
    return std::min(limit_dist, static_cast<float>((1 + std::sqrt(std::pow(current_point(0,0)-proj_robot(0,0),2) + std::pow(current_point(0,1)-proj_robot(0,1),2)))));
...
void draw_circle(Grid & grid, int const& x, int const& y, int const& radius) // draw a disk of radius 'radius' and centered on (x,y)
{
...
Eigen::MatrixXi readMatrix(std::string filename) // read a textfile with 'space'-separated values
{
    using namespace std;
...
Eigen::Matrix<double, 2, 2> get_derivation_matrix(State const& state_robot, State const& state_attractor, Border const& border)
{
    ...
    Eigen::Matrix<float, 10, 1> point_left_circle  = point_from_initial_to_circle(point_left , state_attractor, border);
    Eigen::Matrix<float, 10, 1> point_right_circle = point_from_initial_to_circle(point_right, state_attractor, border);
    Eigen::Matrix<float, 10, 1> point_up_circle    = point_from_initial_to_circle(point_up   , state_attractor, border);
    Eigen::Matrix<float, 10, 1> point_down_circle  = point_from_initial_to_circle(point_down , state_attractor, border);
