...
void SlamGMapping::init()
{
  ...
  time_start_gmapping = ros::WallTime::now().toSec();
}
...
{
  auto t_start_laser_callback = std::chrono::high_resolution_clock::now();
  ...
  if(addScan(*scan, odom_pose))
  {
    ...
    auto t_end_laser_callback = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = t_end_laser_callback - t_start_laser_callback;
    // std::cout << "T update gmapping: " << diff.count() << std::endl;

        if (true)
        {
            float timestamp_timing = ros::Time::now().toSec();
            std::ofstream my_timing;
            my_timing.open("/home/qolo/catkin_ws/src/process_occupancy_grid/src/Logging/timing_gmapping_"+std::to_string(static_cast<int>(std::round(time_start_gmapping)))+".txt", std::ios::out | std::ios_base::app);
            my_timing << timestamp_timing << "," << diff.count() << "\n";
            my_timing.close();
        }
    }
    else
    {
      ROS_INFO("Time diff %f", (scan->header.stamp - last_map_update).toSec());
    }
  } else
