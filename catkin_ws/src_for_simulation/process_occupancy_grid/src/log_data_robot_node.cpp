...
/**
 * This node is used during experiments with the real Ridgeback or in simulation with Gazebo depending on the value of the is_simulation parameter
 * The goal of this node is to log the position and velocity of the Ridgeback (/odometry/filtered topic or /gazebo/link_states topic). It also logs:
 * Velocity commands sent to the Ridgeback (/cmd_vel topic)
 * LaserScan messages sent to the SLAM algorithm (/scan_multi topic)
 * The position of the attractor (/attractor topic)
 * People's estimated position in the map (/pose_people_map_filtered topic)
 * Metadata information about the current occupancy grid (/map_info topic)
 * Logs are saved in .txt file that have to be opened with Python to get relevant plots with matplotlib
 * Logs generated by this node are used by the interactive Python script to see what happened during the experiments
 */
