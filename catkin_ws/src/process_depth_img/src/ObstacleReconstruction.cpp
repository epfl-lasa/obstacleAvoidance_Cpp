    ...
    for (float x=limits(0,0); x <= limits(1,0); x += limits(4,0)) // x direction of the grid
    {
        ...
        for (float y=limits(2,0); y <= limits(3,0); y += limits(4,0)) // y direction of the grid
        {
            ...
            if (!flag) // if not inside an obstacle
            {
                ...
                State next_eps = next_step_special( state_robot, state_attractor, border_out); // compute special velocity with transform in circle space
...
void compute_stream_border(Eigen::Matrix<float, 5, 1> const& limits, State const& state_attractor, Blob const& obstacle)
{
    // Compute the velocity command at the initial time for all points of a [x,y] grid
    // Save the result in a txt file and then use a Python script to plot the vector field (quiver) with matplotlib
    //const int number_obstacles = mat_obs.cols();
...
State next_step_special(State const& state_robot, State const& state_attractor, Border const& border)
{
    // Compute all the steps for a single step and a single obstacle
    ...
    if (verbose)
    {
        std::cout << "Projected   " << gamma_norm_proj(4,0) << " " << gamma_norm_proj(5,0) << std::endl;
    ...
    {
        State output; output << std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::quiet_NaN(), 0;
        return output;
    }
    ...
    //std::cout << "Step 3" << std::endl;
    // Compute attractor function
    State f_eps = attractor_circle_frame - robot_circle_frame;
    //f_eps =  f_eps.normalized();


    // Several steps to get D(epsilon) matrix
    float lamb_r = 1 - (1/gamma_circle_frame);
    float lamb_e = 1 + (1/gamma_circle_frame);
    Eigen::Matrix<float, number_states, number_states> D_eps = D_epsilon( lamb_r, lamb_e);
    ...
    State r_eps_vector = ref_vec_circle_frame;
    State gradient_vector = gradient( robot_circle_frame, obs);
    Eigen::Matrix<float, number_states, number_states-1> ortho_basis = gram_schmidt( gradient_vector);
    Eigen::Matrix<float, number_states, number_states> E_eps = E_epsilon( r_eps_vector, ortho_basis);
    ...
    // Compute epsilon_dot
    State velocity_circle_space = epsilon_dot( M_eps, f_eps, robot_circle_frame, obs);
...
Eigen::Matrix<float, 1, 3> get_distances_surface(Eigen::Matrix<float, 1, 2> const& proj_robot, Eigen::Matrix<float, 1, 2> const& proj_attractor, Border const& border)
{
    ...
    case 1:
        ...
        else
        {
            ...
            add_down_attractor =              (proj_attractor(0,1) - cell_proj_attractor(1,0))*direction + 0.5 * size_cell;
            add_up_attractor   = size_cell - ((proj_attractor(0,1) - cell_proj_attractor(1,0))*direction + 0.5 * size_cell);
        }
        break;
